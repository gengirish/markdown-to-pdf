...
def _passport_identity(principal: Principal) -> tuple[str, str]:
    ...
    if not seed:
        ...
        seed = _slugify(re.sub(r"^user_", "", principal.clerk_user_id)) or _USERNAME_FALLBACK
        display_name = seed
...
def claim_credential(
    ...
    with get_db() as session:
        ...
        passport = session.query(Passport).filter_by(clerk_user_id=principal.clerk_user_id).first()
        if not passport:
            username, display_name = _passport_identity(principal)
            passport = Passport(
